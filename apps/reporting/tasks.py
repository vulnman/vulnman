...
def do_create_report(report_pk):
    ...
    template_pks = Template.objects.filter(vulnerability__project=report.project, vulnerability__status=Vulnerability.STATUS_VERIFIED).order_by('-vulnerability__cvss_score').values_list("pk", flat=True)
    unique_pks = []
