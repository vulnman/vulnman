...
class PentestInformationViewSetTestCase(APITestCase, VulnmanAPITestCaseMixin):
    def setUp(self):
        self.init_mixin()

    def test_report_update(self):
        #report = self.create_instance(models.PentestReport, project=self.project)
        url = self.get_url("api:v1:report-information-detail", pk=self.project.reportinformation.pk)
        data = {"evaluation": "This is a *test*", "recommendation": "This is another **test**", "author": self.project_pentester.pk}
